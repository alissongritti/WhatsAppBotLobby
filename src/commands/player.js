...
async function entrar({ msg, chat, parametro, senderId, nome, groupId }) {
  ...
  const partidaAlvo = await resolverPartidaAlvo({ msg, chat, parametro, groupId, acao: "entrar" });
  ...
  if (numTitulares < maxPlayers) {
    ...
    if (vagasRestantes === 0) {
      ...
      if (partidaAlvo.horario) textoFinal += `⏰ *Horário:* ${partidaAlvo.horario}\n`;
      ...
      if (partidaAlvo.horario) textoParcial += `⏰ *Horário:* ${partidaAlvo.horario}\n`;
      textoParcial += `\n${await gerarListaTexto(partidaAlvo.id, maxPlayers)}`;
...
async function sair({ msg, chat, parametro, senderId, nome, groupId }) {
  ...
  if (parametro) {
    ...
    if (!partidaAlvo) {
      await msg.reply(`Não encontrei a partida #${idBuscado} ou ela já foi fechada.`);
      return;
    }
  } else {
    const partidas = await partidaService.getPartidasDoJogador(groupId, senderId);
    if (partidas.length === 0) {
      await msg.reply("Burro ou leigo? Você não está em nenhuma partida aberta...");
      return;
      ...
      partidas.forEach((p) => { textoAviso += `ID #${p.numero_lobby} - ${p.titulo}\n`; });
  ...
  if (!registro) {
    await msg.reply(`Você não está na lista da partida #${partidaAlvo.numero_lobby}.`);
    return;
  }
  ...
  if (registro.papel === "SUPLENTE") {
    await chat.sendMessage(`${nome} cansou de esperar e saiu dos suplentes da partida #${partidaAlvo.numero_lobby}.`);
    return;
  }
  ...
  if (!temAlguem) {
    ...
    await chat.sendMessage(`Todo mundo arregou. A partida #${partidaAlvo.numero_lobby} foi cancelada!`);
    return;
  }

  let textoSair = `🎮 *${partidaAlvo.tipo} #${partidaAlvo.numero_lobby}: ${partidaAlvo.titulo} ATUALIZADA* 🎮\n`;
  if (partidaAlvo.horario) textoSair += `⏰ *Horário:* ${partidaAlvo.horario}\n`;
...
async function resolverPartidaAlvo({ msg, parametro, groupId, acao }) {
  ...
  abertas.forEach((p) => { textoAviso += `ID #${p.numero_lobby} - ${p.titulo} (${p.tipo})\n`; });
