...
async function meunick({ msg, parametro, senderId }) {
  if (!parametro) {
    ...
    if (nickRow) {
      await msg.reply(`Seu nick atual é: *${nickRow.nome}*\n\nPara mudar, mande: *!meunick NovoNome*`);
    } else {
      await msg.reply("Você ainda não definiu um nick personalizado.\nComo quer ser chamado? Exemplo: *!meunick Sonzera*");
    }
    return;
  }

  if (parametro.length > 15) {
    await msg.reply("Nick muito grande, emocionado! Escolha um com até 15 letras.");
    return;
  }

  await jogadorService.setNick(senderId, parametro);
  await msg.reply(`✅ Nick atualizado! A partir de agora vou te chamar de *${parametro}*.`);
}
...
module.exports = { meunick, status, comandos };
